...
Ryan's clock is at `192.168.1.245`. Settings UI: http://192.168.1.245/. Running `1.4.2` / `CW_TICKER` on `cw-cf-0x07` (Canvas), pointed at the panel worker. The worker rotates four faces (moon, weather, agenda, gameday) on five minute slots, with anything at urgency 5 or over holding the panel instead; the note here that weather rests permanently is obsolete as of moon-canvas#12, which added the time term the resolver never had. `CW_TICKER` carries clockwise#15 (scroller state survives a content change when the document's `face` is unchanged), clockwise#16 (renderText erases the PREVIOUS extent, so a narrowing string cannot leave a ghost), clockwise#17 (`GET /measure`) and clockwise#18 (`SCROLL_TICKER`).

**There are three scroll modes and only one of them is what you want.** `once` runs a single pass and then PARKS at the head, so a long line spends the rest of its life showing its own first 62px, which on a wall is indistinguishable from being truncated. `loop` is a ping-pong. `ticker` is Times Square: one direction, wrapping, forever, and it works by drawing the string twice, `textW + SCROLL_TICKER_GAP` apart, so the frame at `-pitch` is pixel identical to the frame at `0` and the seam is invisible. Every face in moon-canvas uses `ticker`. `once` and `loop` stay for hand written documents and are exercised by `/hostile.json`. The `192.168.1.44` address in earlier notes is stale.
